...
def login():
    ...
    if request.method == "POST":
        usuario = request.form["usuario"] #primeira declaração da variavel USUARIO, e ela é digitada no formulario(login.html) pelo usuario
        ...
        if usuario == "jose" and senha == "1234": #verifica se as credenciais (jose e 1234)são corretas
            ...
            return redirect(url_for("painel")) #encontre a url (ruta painel) a função painel (url_for("painel")), mande ao usuario a outra pag (redirect) retorne tudo isso (return)
...
#função para criar o banco, se executa alem do app.run que é para as rotas
def banco_novo():       
...
#a ruta serve para acessar aos usuarios
@app.route("/usuarios")   
def usuarios():
      conexao = sqlite3.connect("crud.db")
      cursor = conexao.cursor()
...
# as / separam endereços 
# <> e o valor duma var que vem da URL, int indica o tipo de dado e : separa isso 
# entender porque é :id_usuario
#entende ro uso da virgula no (id_usuario,),
